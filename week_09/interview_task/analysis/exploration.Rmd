...
#### cancer type by age for top ones in borders

```{r}
# top cancers females in borders
...
Or use incidences in "All ages" to find top 10 by number of incidences alone

```{r}
top_cancers_borders_females_inc <- comb_5_hb_inc_rate_age %>% 
  filter(hb == hb_borders &
         sex == "Females" & 
           cancer_site != "All cancer types") %>%
  summarise(incidences = max(incidences_all_ages), .by = cancer_site) %>% 
  arrange(desc(incidences)) %>% 
  slice_max(incidences, n = 10)

top_cancers_borders_females_inc_list <- top_cancers_borders_females_inc %>% 
  pull(cancer_site)
```
...
ggsave("tile_female_borders_inc_by_cancer_age.png", width = 15, height = 10, units = "cm")
```

Try with top 10 by total incidences (not mean easr)

```{r}
top_cancers_borders_females_inc %>% 
  ggplot() +
  geom_col(aes(x = incidences, y = reorder(cancer_site, incidences)))

comb_5_hb_inc_rate_age %>% 
  filter(sex == "Females" & cancer_site %in% top_cancers_borders_females_inc_list) %>% 
  mutate(cancer_site = factor(cancer_site, levels = c(
    "Breast", "Non-melanoma skin cancer", 
    "Basal cell carcinoma of the skin",
    "Trachea, bronchus and lung", "Colorectal cancer", 
    "Colon", "Carcinoma in situ of the cervix uteri",  
    "Squamous cell carcinoma of the skin", "Uterus", 
...
## check quality of dfs used

```{r, eval = FALSE}
# used the following dfs
# (loaded to env by running presentation_nb.Rmd)
incidence_network # easr and CIs, sex, cancer_site, year > 2011
incidence_scot
incidence_hb
comb_5yr_incidence_hb # easr (check for NAs, mean(na.rm=TRUE), incidence rate by age brackets, cancer_site, sex
```

```{r}
comb_5yr_incidence_hb %>%
  filter(is.na(easr)) # 0 NAs
# used mean(easr), no NAs so no worries about this
# (not checked for normal distribution of easr though)

incidence_hb %>%
  filter(hb == hb_borders) %>% # 3400 rows
  filter(!is.na(easr_lower95pc_confidence_interval_qf) | !is.na(easr_upper95pc_confidence_interval_qf)) %>% # 2,507 NAs in CIs
  distinct(cancer_site, sex) 

incidence_network %>%
  filter(region == "South East of Scotland") %>% # 3400 rows
  filter(!is.na(easr_lower95pc_confidence_interval_qf) | !is.na(easr_upper95pc_confidence_interval_qf)) %>% # 554 NAs in CIs
  distinct(cancer_site, sex) 

incidence_scot %>% # 3400 rows
  filter(!is.na(easr_lower95pc_confidence_interval_qf) | !is.na(easr_upper95pc_confidence_interval_qf)) %>% # 87 NAs in CIs
  distinct(cancer_site, sex) 
```

These NAs in CIs are for diff types of cancers ~ sexes. Non are for non-melanoma skin cancer, which is what I've used these CIs to plot in presentation.
